...
class Hydrator
{
    ...
    public function getPost($row = array())
    {
        ...
        $author = isset($row["from_id"]) ? $this->vkService->userGet($row["from_id"]) : null;
        $owner = isset($row["to_id"]) ? $this->vkService->userGet($row["to_id"]) : null;
        $createdAt = (new \DateTime())->setTimestamp($row["date"]);
